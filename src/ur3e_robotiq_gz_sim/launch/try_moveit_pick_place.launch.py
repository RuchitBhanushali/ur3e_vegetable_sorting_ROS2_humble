...
def generate_launch_description():
    ...
    joint_controllers_file = os.path.join(
        get_package_share_directory('ur3e_robotiq_gz_sim'), 'config', 'ur3e_robotiq_ros2_controllers.yaml'
    )
    ...
    world_sdf = PathJoinSubstitution(
        [FindPackageShare("ur3e_robotiq_gz_sim"), "worlds", "pick_place_table_world.sdf"]
    )
    ...
    ld.add_action(y_arg)
    ld.add_action(z_arg)
    ld.add_action(gz_launch_with_gui)
